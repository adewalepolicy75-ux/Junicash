...
import { FiUser, FiShield, FiFileText, FiCreditCard, FiHelpCircle, FiLink, FiSliders, FiEyeOff, FiMoon, FiLogOut } from 'react-icons/fi'
    ...
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
        <div style={{ position: 'relative', display: 'inline-block', marginBottom: '1rem' }}>
          <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: '#6d28d9', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: '800', fontSize: '2rem', margin: '0 auto', overflow: 'hidden', border: '3px solid #ddd6fe' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span style={{ color: '#6d28d9', fontSize: '1rem' }}>◈</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1.5rem' }}>
        {menuItems.map((row, rowIndex) => (
          row.map((item, colIndex) => (
            <div key={rowIndex + '-' + colIndex} onClick={item.toggle ? undefined : () => {}} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '1rem 1.25rem', cursor: item.toggle ? 'default' : 'pointer' }}>
