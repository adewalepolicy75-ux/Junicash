...
function Signup() {
  ...
  const otpBox = { width: '46px', height: '52px', borderRadius: '12px', border: '2px solid #ddd6fe', fontSize: '1.4rem', fontWeight: '800', textAlign: 'center', outline: 'none', color: '#0a0a0f', background: '#f5f3ff' }
        ...
        <div style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '20px', padding: '2.5rem', width: '100%', maxWidth: '420px' }}>
          ...
          {step === 1 && (
            <>
              ...
              <form onSubmit={handleSendOTP}>
                ...
                <input type='password' name='password' placeholder='Min 6 characters' value={form.password} onChange={handleChange} style={input} />
          ...
          {step === 2 && (
            <>
              ...
              <p style={{ textAlign: 'center', marginTop: '1rem', fontSize: '0.85rem', color: '#9ca3af', cursor: 'pointer' }} onClick={() => { setStep(1); setError('') }}>
                Back to sign up
              </p>
            </>
          )}
