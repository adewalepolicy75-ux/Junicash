...
function Dashboard() {
  ...
  const recent = [...transactions]
    .filter(tx => overviewFilter === 'all' || tx.type === overviewFilter)
            ...
            <div style={{ background: '#ffffff', borderRadius: '16px', border: '1px solid #e5e7eb', padding: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
                {['all', 'income', 'expense'].map(f => (
                  <button key={f} onClick={() => setOverviewFilter(f)} style={{ padding: '0.35rem 0.85rem', borderRadius: '20px', border: 'none', cursor: 'pointer', fontSize: '0.8rem', fontWeight: '600', background: overviewFilter === f ? '#6d28d9' : '#f3f4f6', color: overviewFilter === f ? '#ffffff' : '#6b7280', textTransform: 'capitalize' }}>
                    {f}
                  </button>
                ))}
              </div>
